...
from constants import STRING_KEY_NUM_MINUTE_DICT, CLOSE_COLUMN, VOLUME_COLUMN
import pandas as pd
import math
import os.path
...
class DataManager:
    ...
    def get_historical_data_DataFrame(self, symbol, kline_size) -> pd.DataFrame:
        ...
        file_path = self.file_path_manager_obj.get_historical_data_csv_file_path(symbol, kline_size)
    ...
    def __get_all_binance(self, symbol, kline_size, save = False):
        ...
        oldest_point, newest_point = self.__minutes_of_new_data(symbol, kline_size, data_df, source = "binance")
        delta_min = (newest_point - oldest_point).total_seconds()/60
        available_data = math.ceil(delta_min/STRING_KEY_NUM_MINUTE_DICT[kline_size])
        ...
        klines = self.binance_client.get_historical_klines(symbol, kline_size, oldest_point.strftime("%d %b %Y %H:%M:%S"), newest_point.strftime("%d %b %Y %H:%M:%S"))
        ...
        columns_to_delete = ['close_time', 'quote_av', 'trades', 'tb_base_av', 'tb_quote_av', 'ignore']            
        ...
        if kline_size in ['1h', '4h', '1d']:
            ...
            self.pre_process_obj.add_high_to_low_precent_change_column(data_df, kline_size, one_minute_df)
    ...
    def download_historical_data(self, time_frames : list = None, coin_pairs : list = None):
        time_frames = ["1m", "5m", "15m", "1h", "4h", "1d"] if time_frames is None else time_frames
        coin_pairs = ["BTCUSDT", "ETHUSDT", "LTCUSDT", "ADAUSDT"] if coin_pairs is None else coin_pairs
...
if __name__ == "__main__":
    dm = DataManager()
    # df = dm.get_historical_data_DataFrame('BTCUSDT', '1d')
    dm.download_historical_data()